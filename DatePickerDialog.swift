...
class DatePickerDialog: UIView {
    ...
    private func createContainerView() -> UIView {
        ...
        // Set minimum date to be rounded to :30 or :00
        let dateComponents: NSDateComponents = NSCalendar.currentCalendar().components(NSCalendarUnit.Minute, fromDate: NSDate())
        let minutes = dateComponents.minute
        let minutesRounded = (minutes / 30) * 30
        var roundedDate: NSDate = NSDate.init(timeInterval: 60.0 * (Double(minutesRounded - minutes)), sinceDate: NSDate())
        let timeInterval: NSTimeInterval = 1800
        roundedDate = roundedDate.dateByAddingTimeInterval(timeInterval)
        
        self.datePicker.date = roundedDate
        self.datePicker.minimumDate = roundedDate
        self.datePicker.minuteInterval = 30
        let language = NSBundle.mainBundle().preferredLocalizations.first! as String
        self.datePicker.timeZone = NSTimeZone.localTimeZone()
        if language == "en" {
            self.datePicker.locale = NSLocale(localeIdentifier: "en_FR")
        } else if language == "fr" {
            self.datePicker.locale = NSLocale(localeIdentifier: "fr_FR")
        }
